...
# Funzione per ottenere una lista piatta di tutte le finiture
TUTTE_LE_FINITURE = sorted([f for lista in DB_FINITURE_LEGNO.values() for f in lista if f])

# =========================================================
# 1. DIZIONARI E DATABASE
# =========================================================

# --- LOGICA DI SELEZIONE FINITURE (DA INSERIRE DOPO LA SCELTA DEL COMPONENTE) ---

# Verifichiamo se l'utente ha selezionato un componente che richiede finitura
tipo_parte = st.session_state.get('selectbox_part', "")

if tipo_parte in ["WOODCOMP", "ASSEMBLY"]:
    st.divider()
    st.subheader(f"🎨 Finiture per {tipo_parte}")
    
    # Se è un pezzo singolo (WOODCOMP)
    if tipo_parte == "WOODCOMP":
        col_cat, col_fin = st.columns(2)
        with col_cat:
            cat = st.selectbox("Categoria Legno", list(DB_FINITURE_LEGNO.keys()), key="cat_wood")
        with col_fin:
            # Mostra solo le finiture della categoria scelta
            finitura_scelta = st.selectbox("Specifica Finitura", DB_FINITURE_LEGNO[cat], key="fin_wood")
        
        # Salviamo il risultato
        st.session_state['finitura_finale'] = finitura_scelta

    # Se è un assieme (ASSEMBLY) - Gestione fino a 3 finiture
    elif tipo_parte == "ASSEMBLY":
        finiture_selezionate = []
        
        # Creiamo 3 righe per le 3 possibili finiture
        for i in range(1, 4):
            expander_label = f"Finitura {i}" if i == 1 else f"Finitura {i} (Opzionale)"
            with st.expander(expander_label, expanded=(i==1)):
                c1, c2 = st.columns(2)
                with c1:
                    cat_i = st.selectbox(f"Categoria {i}", ["-"] + list(DB_FINITURE_LEGNO.keys()), key=f"cat_as_{i}")
                with c2:
                    if cat_i != "-":
                        fin_i = st.selectbox(f"Specifica Finitura {i}", DB_FINITURE_LEGNO[cat_i], key=f"fin_as_{i}")
                        if fin_i:
                            finiture_selezionate.append(fin_i)
                    else:
                        st.info("Seleziona una categoria")
        
        # Uniamo le finiture con il separatore
        st.session_state['finitura_finale'] = " | ".join(finiture_selezionate)

    # Visualizzazione anteprima stringa finitura
    if st.session_state.get('finitura_finale'):
        st.success(f"Stringa Finitura: `{st.session_state['finitura_finale']}`")
