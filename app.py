...
# --- DATABASE FINITURE ---
# Ho mantenuto la tua struttura originale, è perfetta per la ricerca
DB_FINITURE_LEGNO = {
...
def gestisci_ricerca(prompt_testo):
    """Funzione interna per filtrare il database e scegliere la finitura."""
    while True:
        termine = input(f"\n{prompt_testo} (es. 'Rovere' o codice): ").strip().upper()
        
        # Filtriamo le opzioni che contengono il termine cercato
        opzioni = []
        for cat, finiture in DB_FINITURE_LEGNO.items():
            for f in finiture:
                if f and termine in f.upper():
                    opzioni.append(f)
        
        if not opzioni:
            print(f"(!) Nessuna corrispondenza per '{termine}'. Prova con un'altra parola.")
            continue
        ...
        scelta = input("Seleziona il numero (o premi Invio per cercare ancora): ")
        if scelta == "" : continue
        
        try:
            return opzioni[int(scelta)]
        except (ValueError, IndexError):
            print("(!) Selezione non valida.")

def main():
    print("--- SISTEMA INSERIMENTO FINITURE ---")
    print("Scegli la modalità:")
    print("1) WOODCOMP (1 finitura)")
    print("2) ASSEMBLY (fino a 3 finiture)")
    ...
    if modo == "1":
        # Modalità Singola
        f = gestisci_ricerca("Cerca finitura WOODCOMP")
        scelte_finali.append(f)
    
    elif modo == "2":
        # Modalità Multipla
        for i in range(1, 4):
            f = gestisci_ricerca(f"Cerca finitura {i} di 3 per ASSEMBLY")
            scelte_finali.append(f)
            if i < 3:
                altro = input("Vuoi aggiungere un'altra finitura all'assieme? (s/n): ").lower()
                if altro != 's':
                    break
    else:
    ...
    # Risultato finale stampato a video
    risultato = " | ".join(scelte_finali)
    
    print("\n" + "="*60)
    print("ECCO LA TUA STRINGA DA COPIARE:")
    print(risultato)
    print("="*60)
    print("\nLavoro terminato. Puoi selezionare la stringa qui sopra e copiarla.")
