...
st.divider()
st.subheader("🎨 Selezione Finiture")

# Scegli il tipo di componente per decidere quante finiture mostrare
tipo_comp = st.radio("Tipo Componente", ["WOODCOMP", "ASSEMBLY"], horizontal=True)

finiture_selezionate = []

if tipo_comp == "WOODCOMP":
    # Menu a tendina singolo con ricerca
    scelta = st.selectbox("Seleziona Finitura", [""] + TUTTE_LE_FINITURE, key="fin_wood")
    if scelta:
        finiture_selezionate.append(scelta)

else:
    # Mostra 3 menu a tendina per l'ASSEMBLY
    col1, col2, col3 = st.columns(3)
    with col1:
        f1 = st.selectbox("Finitura 1", [""] + TUTTE_LE_FINITURE, key="fin_1")
    with col2:
        f2 = st.selectbox("Finitura 2", [""] + TUTTE_LE_FINITURE, key="fin_2")
    with col3:
        f3 = st.selectbox("Finitura 3", [""] + TUTTE_LE_FINITURE, key="fin_3")
    
    # Filtriamo quelle vuote
    finiture_selezionate = [f for f in [f1, f2, f3] if f]

# Risultato finale per questa sezione
stringa_finiture = " | ".join(finiture_selezionate)

if stringa_finiture:
    st.info(f"Finiture scelte: **{stringa_finiture}**")
